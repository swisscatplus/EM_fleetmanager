...
local_path = 'config/robot_params.ini'
...
def set_ros_pose_cov_stamped(pose_values):
    ...
    pose_cov_stamped.pose.pose.position.y = float(pose_values[1])'config/robot_params.ini'
...
def setup_config_and_logger(logger_name='DEFAULT_NAME', log_level='INFO', path):
    # Create a ConfigParser object
    config = configparser.ConfigParser()

    to_pkg_dir = os.path.join(os.getcwd(), package_path_from_src,
    config_dir = os.path.join(to_pkg_dir ,local_path)
