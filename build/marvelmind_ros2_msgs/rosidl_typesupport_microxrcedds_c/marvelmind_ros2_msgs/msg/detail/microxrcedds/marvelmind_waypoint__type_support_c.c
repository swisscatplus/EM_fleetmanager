// generated from rosidl_typesupport_microxrcedds_c/resource/idl__type_support_c.c.em
// with input from marvelmind_ros2_msgs:msg/MarvelmindWaypoint.idl
// generated code does not contain a copyright notice
#include "marvelmind_ros2_msgs/msg/detail/marvelmind_waypoint__rosidl_typesupport_microxrcedds_c.h"


#include <stdint.h>
#include <stdio.h>
#include <string.h>
#include "rosidl_typesupport_microxrcedds_c/identifier.h"
#include "rosidl_typesupport_microxrcedds_c/message_type_support.h"
#include "marvelmind_ros2_msgs/msg/rosidl_typesupport_microxrcedds_c__visibility_control.h"
#include "marvelmind_ros2_msgs/msg/detail/marvelmind_waypoint__struct.h"
#include "marvelmind_ros2_msgs/msg/detail/marvelmind_waypoint__functions.h"

#ifndef _WIN32
# pragma GCC diagnostic push
# pragma GCC diagnostic ignored "-Wunused-parameter"
# ifdef __clang__
#  pragma clang diagnostic ignored "-Wdeprecated-register"
#  pragma clang diagnostic ignored "-Wreturn-type-c-linkage"
# endif
#endif
#ifndef _WIN32
# pragma GCC diagnostic pop
#endif

#define MICROXRCEDDS_PADDING sizeof(uint32_t)

// includes and forward declarations of message dependencies and their conversion functions

#if defined(__cplusplus)
extern "C"
{
#endif


// forward declare type support functions


typedef marvelmind_ros2_msgs__msg__MarvelmindWaypoint _MarvelmindWaypoint__ros_msg_type;

static bool _MarvelmindWaypoint__cdr_serialize(
  const void * untyped_ros_message,
  ucdrBuffer * cdr)
{
  (void) untyped_ros_message;
  (void) cdr;

  bool rv = false;

  if (!untyped_ros_message) {
    return false;
  }

  _MarvelmindWaypoint__ros_msg_type * ros_message = (_MarvelmindWaypoint__ros_msg_type *)(untyped_ros_message);
  (void)ros_message;

  // Member: total_items
  rv = ucdr_serialize_uint8_t(cdr, ros_message->total_items);
  // Member: item_index
  rv = ucdr_serialize_uint8_t(cdr, ros_message->item_index);
  // Member: movement_type
  rv = ucdr_serialize_uint8_t(cdr, ros_message->movement_type);
  // Member: param1
  rv = ucdr_serialize_int16_t(cdr, ros_message->param1);
  // Member: param2
  rv = ucdr_serialize_int16_t(cdr, ros_message->param2);
  // Member: param3
  rv = ucdr_serialize_int16_t(cdr, ros_message->param3);

  return rv;
}

static bool _MarvelmindWaypoint__cdr_deserialize(
  ucdrBuffer * cdr,
  void * untyped_ros_message)
{
  (void) cdr;

  bool rv = false;

  if (!untyped_ros_message) {
    return false;
  }
  _MarvelmindWaypoint__ros_msg_type * ros_message = (_MarvelmindWaypoint__ros_msg_type *)(untyped_ros_message);
  (void)ros_message;

  // Field name: total_items
  rv = ucdr_deserialize_uint8_t(cdr, &ros_message->total_items);
  // Field name: item_index
  rv = ucdr_deserialize_uint8_t(cdr, &ros_message->item_index);
  // Field name: movement_type
  rv = ucdr_deserialize_uint8_t(cdr, &ros_message->movement_type);
  // Field name: param1
  rv = ucdr_deserialize_int16_t(cdr, &ros_message->param1);
  // Field name: param2
  rv = ucdr_deserialize_int16_t(cdr, &ros_message->param2);
  // Field name: param3
  rv = ucdr_deserialize_int16_t(cdr, &ros_message->param3);
  return rv;
}

ROSIDL_TYPESUPPORT_MICROXRCEDDS_C_PUBLIC_marvelmind_ros2_msgs
size_t get_serialized_size_marvelmind_ros2_msgs__msg__MarvelmindWaypoint(
  const void * untyped_ros_message,
  size_t current_alignment)
{
  if (!untyped_ros_message) {
    return 0;
  }

  const _MarvelmindWaypoint__ros_msg_type * ros_message = (const _MarvelmindWaypoint__ros_msg_type *)(untyped_ros_message);
  (void)ros_message;

  const size_t initial_alignment = current_alignment;

  // Member: total_items
  {
    const size_t item_size = sizeof(ros_message->total_items);
    current_alignment += ucdr_alignment(current_alignment, item_size) + item_size;
  }
  // Member: item_index
  {
    const size_t item_size = sizeof(ros_message->item_index);
    current_alignment += ucdr_alignment(current_alignment, item_size) + item_size;
  }
  // Member: movement_type
  {
    const size_t item_size = sizeof(ros_message->movement_type);
    current_alignment += ucdr_alignment(current_alignment, item_size) + item_size;
  }
  // Member: param1
  {
    const size_t item_size = sizeof(ros_message->param1);
    current_alignment += ucdr_alignment(current_alignment, item_size) + item_size;
  }
  // Member: param2
  {
    const size_t item_size = sizeof(ros_message->param2);
    current_alignment += ucdr_alignment(current_alignment, item_size) + item_size;
  }
  // Member: param3
  {
    const size_t item_size = sizeof(ros_message->param3);
    current_alignment += ucdr_alignment(current_alignment, item_size) + item_size;
  }

  return current_alignment - initial_alignment;
}

static uint32_t _MarvelmindWaypoint__get_serialized_size(const void * untyped_ros_message)
{
  return (uint32_t)(
    get_serialized_size_marvelmind_ros2_msgs__msg__MarvelmindWaypoint(
      untyped_ros_message, 0));
}

ROSIDL_TYPESUPPORT_MICROXRCEDDS_C_PUBLIC_marvelmind_ros2_msgs
size_t max_serialized_size_marvelmind_ros2_msgs__msg__MarvelmindWaypoint(
  bool * full_bounded,
  size_t current_alignment)
{
  (void) current_alignment;
  *full_bounded = true;

  const size_t initial_alignment = current_alignment;

  // Member: total_items
  current_alignment += ucdr_alignment(current_alignment, sizeof(uint8_t)) + sizeof(uint8_t);
  // Member: item_index
  current_alignment += ucdr_alignment(current_alignment, sizeof(uint8_t)) + sizeof(uint8_t);
  // Member: movement_type
  current_alignment += ucdr_alignment(current_alignment, sizeof(uint8_t)) + sizeof(uint8_t);
  // Member: param1
  current_alignment += ucdr_alignment(current_alignment, sizeof(int16_t)) + sizeof(int16_t);
  // Member: param2
  current_alignment += ucdr_alignment(current_alignment, sizeof(int16_t)) + sizeof(int16_t);
  // Member: param3
  current_alignment += ucdr_alignment(current_alignment, sizeof(int16_t)) + sizeof(int16_t);

  return current_alignment - initial_alignment;
}

static size_t _MarvelmindWaypoint__max_serialized_size()
{
  bool full_bounded;
  return max_serialized_size_marvelmind_ros2_msgs__msg__MarvelmindWaypoint(&full_bounded, 0);
}

static message_type_support_callbacks_t __callbacks_MarvelmindWaypoint = {
  "marvelmind_ros2_msgs::msg",
  "MarvelmindWaypoint",
  _MarvelmindWaypoint__cdr_serialize,
  _MarvelmindWaypoint__cdr_deserialize,
  _MarvelmindWaypoint__get_serialized_size,
  get_serialized_size_marvelmind_ros2_msgs__msg__MarvelmindWaypoint,
  _MarvelmindWaypoint__max_serialized_size
};

static rosidl_message_type_support_t _MarvelmindWaypoint__type_support = {
  ROSIDL_TYPESUPPORT_MICROXRCEDDS_C__IDENTIFIER_VALUE,
  &__callbacks_MarvelmindWaypoint,
  get_message_typesupport_handle_function,
};

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_microxrcedds_c, marvelmind_ros2_msgs, msg, MarvelmindWaypoint)() {
  return &_MarvelmindWaypoint__type_support;
}

#if defined(__cplusplus)
}
#endif
